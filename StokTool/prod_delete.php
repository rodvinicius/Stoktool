...
<body>
    ...
    if(!is_logado()){
        ...
        if(!isset($_POST['nome'])){
            ...
            if($capa==null){
                $q = "update produto SET nome='$nome', valor=$valor, qtd_estoq=$qtd_estoq, imagem='$capaOriginal', ativo='false' where COD_PROD=$cod ";

            }else{
                $q = "update produto SET nome='$nome', valor=$valor, qtd_estoq=$qtd_estoq, imagem='$capa', ativo='false' where COD_PROD=$cod ";
            ...
            if(empty($nome) || empty($valor || empty($qtd_estoq)) ){
                ...
                if($banco->query($q)){
                    $mensagem = "Produto $nome excluído.";
